...
function showResult1($id){
	...
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM result where subid='$reid' ";
	$stmt= $mysqli->query($query);
	return $stmt;
...
function create_subject($cshort,$cfull,$semester,$sub1,$sub2,$sub3,$sub4,$sub5,$sub6){
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into subject(cshort,cfull,semester,sub1,sub2,sub3,sub4,sub5,sub6)values(?,?,?,?,?,?,?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sssssssss',$cshort,$cfull,$semester,$sub1,$sub2,$sub3,$sub4,$sub5,$sub6);
				$stmt->execute();
				echo "<script>alert('Course Added Successfully')</script>";
	...
	function create_result($cshort,$cfull,$semester,$Sid,$name,$sub1,$cgpa1,$sub2,$cgpa2,$sub3,$cgpa3,$sub4,$cgpa4,$sub5,$cgpa5){
			...
			$db = Database::getInstance();
			$mysqli = $db->getConnection();
			$query = "insert into result(cshort,cfull,semester,Sid,name,sub1,cgpa1,sub2,cgpa2,sub3,cgpa3,sub4,cgpa4,sub5,cgpa5)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			$stmt= $mysqli->prepare($query);
			if(false===$stmt){
				...
				$stmt->bind_param('sssssssssssssss',$cshort,$cfull,$semester,$Sid,$name,$sub1,$cgpa1,$sub2,$cgpa2,$sub3,$cgpa3,$sub4,$cgpa4,$sub5,$cgpa5);
				$stmt->execute();
...
function edit_result($sub1,$sub2,$sub3,$sub4,$sub5,$udate,$id){

    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "update result set sub1=?,sub2=? ,sub3=?,sub4=?,sub5=?,update_date=? where subid=?";
	$stmt= $mysqli->prepare($query);
	$stmt->bind_param('ssssi',$sub1,$sub2,$sub3,$sub4,$sub5,$udate,$id);
	$stmt->execute();
    echo '<script>'; 
...
function del_result($reid){

     //echo $id;exit;
    $db = Database::getInstance();
    $mysqli = $db->getConnection();
    $query="delete from result where reid=?";
    $stmt= $mysqli->prepare($query);
    $stmt->bind_param('i',$reid);
	$stmt->execute();
    echo "<script>alert('Result has been deleted')</script>";
   // echo "<script>window.location.href='view-course.php'</script>";
}
